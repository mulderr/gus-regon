...
postSoap :: Maybe SessionKey -> Text -> Document -> IO (Response BL.ByteString)
postSoap mkey url = postWith opts (T.unpack url) . renderLBS def
  where
    ...
    opts = case mkey of
      Nothing -> baseOpts
      Just key -> baseOpts & header "sid" .~ [TE.encodeUtf8 $ key.unSessionKey]

-- | Read SOAP response
--
-- Expects multipart/related; type="application/xop+xml"
readSoap :: Response BL.ByteString -> Either Text Document
readSoap res = do
...
bir11ReadData res responseName resultName =
  ...
  where
    toDoc = 
      first (Bir11ProtocolError . T.pack . show) . parseText def . TL.fromStrict
...
login url apikey = do
  res <- postSoap Nothing url $ msgLogin url apikey
  pure $ case bir11ReadSoap res "zalogujresponse" "zalogujresult" of
    Right x | T.length x == 20 -> Right $ SessionKey x
    Right x -> Left $ Bir11ProtocolError $ "error: expected a session key with length 20 but got something of length: " <> (T.pack . show . T.length $ x)
    Left err -> Left err
...
search url sessionkey searchParams = do
  res <- postSoap (Just sessionkey) url $ msgSearch url searchParams
...
fullReport url sessionkey report = do
  res <- postSoap (Just sessionkey) url $ msgFullReport url report
...
getValue url sessionkey pname = do
  res <- postSoap (Just sessionkey) url $ msgGetValue url pname
...
logout url sessionkey = do
  res <- postSoap (Just sessionkey) url $ msgLogout url sessionkey
  pure $ case bir11ReadSoap res "wylogujresponse" "wylogujresult" of
    Right "true" -> Right True
    Right "false" -> Right False
    Right x -> Left $ Bir11ProtocolError $ "error: expected a boolean result but got: " <> x
    Left err -> Left err
